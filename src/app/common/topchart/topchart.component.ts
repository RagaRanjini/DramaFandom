...
constructor(private gs:DramasGetServicesService){
  //fetchin all dramas, sorting it in descending order respect to likes count.
  //storing top 5 in topdramas
  this.gs.getDramas().subscribe(
    {
      next:(data:any)=>{
        this.dramas=data;
        this.dramas.sort((a: any, b: any) => b.Likes - a.Likes);
        this.topdramas = this.dramas.slice(0, 5);
      },
      error:()=>this.dramas=[]
    }
  )
